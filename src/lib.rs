...
use nemo::{
    ...
    logical::execution::ExecutionEngine, physical::dictionary::value_serializer::TrieSerializer,
};
...
fn load_file(file: String) -> PyResult<NemoProgram> {
    ...
    let program = nemo::io::parser::parse_program(contents)
        .map_err(|err| NemoError::new_err(format!("{}", err)))?;
    Ok(NemoProgram(program))
}

#[pyfunction]
fn load_string(rules: String) -> PyResult<NemoProgram> {
    let program = nemo::io::parser::parse_program(rules)
        .map_err(|err| NemoError::new_err(format!("{}", err)))?;
    Ok(NemoProgram(program))
}
...
impl NemoOutputManager {
    ...
    fn py_new(path: String, overwrite: bool, gzip: bool) -> PyResult<Self> {
        let output_manager = OutputFileManager::try_new(path.into(), overwrite, gzip)
            .map_err(|err| NemoError::new_err(format!("{}", err)))?;
...
impl NemoEngine {
    ...
    fn py_new(program: NemoProgram) -> PyResult<Self> {
        let engine = ExecutionEngine::initialize(program.0)
            .map_err(|err| NemoError::new_err(format!("{}", err)))?;
    ...
    fn result(&self, predicate: NemoIdb) -> PyResult<Vec<Vec<String>>> {
        ...
        while let Some(record) = table.next_record() {
            res.push(record.into_iter().map(|v| String::from_utf8(v.as_ref().to_vec()).unwrap()).collect());
        }
